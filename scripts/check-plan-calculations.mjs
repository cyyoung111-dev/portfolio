...
const {
  ...
  calculateIsaPeriodEstimates, calculateRetirementCashflow, calculateRealizedGainFromTrades,
} = calculations;
...
for (const row of payoff.rows) {
  assert.equal(row.endingAssets, row.beginningAssets + row.investmentReturn + row.additionalInvestment + row.availableIncome - row.livingExpense - (row.educationExpense || 0) - (row.taxExpense || 0) - row.loanPayment - row.otherExpense, `${row.year}년 현금흐름 항등식`);
}
const pensionTransfer = calculateRetirementCashflow({ currentYear: 2030, currentAge: 54, retirementAge: 54, retirementYears: 3, availableAssets: 10_000_000, pensionAssets: 20_000_000, pensionStartAge:55, annualPensionWithdrawal:2_000_000 });
...
assert.equal(pensionTransfer.rows[1].pensionEndingAssets, 18_000_000);
for (const row of pensionTransfer.rows) assert.equal(row.endingAssets, row.beginningAssets + row.investmentReturn + row.additionalInvestment + row.availableIncome - row.livingExpense - (row.educationExpense || 0) - (row.taxExpense || 0) - row.loanPayment - row.otherExpense);
...
assert.equal(accumulationLoan.rows[0].loanDeductedFromAssets, true);
assert.equal(accumulationLoan.rows[0].endingAssets, 20_680_000);
