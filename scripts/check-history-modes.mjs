...
assert.match(gasSource, /var latestPrices = getLatestPriceHistory\(ss, missingCodes, dateStr, throwOnError\)[\s\S]*var nearestFuturePrices = getEarliestPriceHistory\(ss, stillMissingCodes, dateStr, throwOnError\)/, '가격 누락 시 직전값을 우선하고 최초 구간만 가장 가까운 이후값으로 보완해야 합니다.');
