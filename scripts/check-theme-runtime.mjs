...
assert.match(elements.get('fontSettingsBody').innerHTML, /data-theme-action="font-size"/, 'UI 크기 선택 렌더링 실패');
assert.match(elements.get('fontSettingsBody').innerHTML, /UI 크기/, 'UI 크기 안내 렌더링 실패');
...
assert.equal(context.document.documentElement.dataset.uiFontSize, '8', '8단계 UI 크기 적용 실패');
assert.equal(JSON.parse(saved.get('app_font_size')), 8, 'UI 크기 저장 실패');
