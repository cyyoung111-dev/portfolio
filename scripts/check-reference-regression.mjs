...
const loanDates = calculations.validateLoanScheduleDates({startDate:'2023-06-02',schedule:[{date:'2023-04'},...futureSchedule],remainingMonths:440,asOfMonth:'2026-08'});
assert.ok(loanDates.warnings.some(item=>item.includes('먼저 시작')));
assert.ok(loanDates.warnings.some(item=>item.includes('현재월 포함')));
