            ...
            <table style="width:100%;border-collapse:collapse;font-size:.72rem;min-width:400px">
              <thead>
                <tr style="background:var(--s2);border-bottom:1px solid var(--border)">
                  <th style="padding:8px 12px;font-size:.68rem;font-weight:600;color:var(--muted);text-align:left;white-space:nowrap">날짜</th>
                ...
                ${(() => {
                  ...
                  return g.trades.map(t => {
                    ...
                    return `<tr style="border-bottom:1px solid var(--border);background:${isSell?'rgba(239,68,68,.03)':'transparent'}">
                      <td style="padding:7px 12px;color:var(--muted);white-space:nowrap;font-size:.70rem">${t.date||'⚠️없음'}</td>
                      ...
                      <td style="padding:7px 12px;text-align:right;font-family:monospace">${qty.toLocaleString()}</td>
                      <td style="padding:7px 12px;text-align:right;white-space:nowrap;font-family:monospace;color:${isSell?'var(--red-lt)':'var(--green-lt)'}">
                        ${price.toLocaleString()}원
                      </td>
                      <td style="padding:7px 12px;text-align:right;white-space:nowrap;font-family:monospace">${amount.toLocaleString()}원</td>
                      <td style="padding:7px 12px;text-align:right">${pnlCell || `<span style="color:var(--muted)">-</span>`}</td>
