...
function renderAssetView(area) {
  ...
  const n    = Math.max(0, Number(LOAN.remainingMonths || 0));
  const totalLoanMonths = Math.max(0, Number(LOAN.totalMonths || 0));
  const r    = rate / 100 / 12;
  const monthlyInterest = Math.round(bal * r);
  const pmt  = bal > 0 && n > 0
    ? (r > 0 ? Math.round(bal * r / (1 - Math.pow(1+r,-n))) : Math.round(bal / n))
    : 0;
  const totalInterest = pmt * n - bal;
  const elapsedMonths = Math.max(0, totalLoanMonths - n);
  const progressPct   = totalLoanMonths > 0 ? (elapsedMonths / totalLoanMonths * 100).toFixed(1) : '0.0';
  const payoffYear    = totalLoanMonths > 0 ? (LOAN.startYear || _kstYear()) + Math.ceil(totalLoanMonths / 12) : '-';
  ...
  // ── 상단 핵심 지표 카드 4개 ──
  h += `<div class="asset-summary-grid">
    ${[
  ...
  h += `<div class="card-12-p20">
    ...
    <div class="asset-detail-grid asset-loan-grid">
      ${[
