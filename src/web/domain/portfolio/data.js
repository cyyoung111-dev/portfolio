...
// ════════════════════════════════════════════════════════════════
//  ★★★ 날짜 계산 필수 원칙 — new Date() 직접 사용 금지 ★★★
//
//  new Date() / new Date('YYYY-MM-DD') / toISOString() 직접 사용 시
//  로컬 타임존 의존으로 KST 자정(00:00~08:59)에 날짜가 하루 밀리는 버그 발생
//
//  반드시 아래 KST 헬퍼 함수를 사용할 것:
//
//  오늘 날짜 문자열 (YYYY-MM-DD) → _kstTodayStr()
//  오늘 연-월    (YYYY-MM)       → _kstMonthStr()
//  현재 연도     (YYYY)          → _kstYear()
//  현재 월       (1~12)          → _kstNow().getUTCMonth() + 1
//  N일 전/후 날짜               → _kstDateOffset(dateStr, n)
//  문자열 → Date 파싱           → _kstDateFromStr(dateStr)
//  Date 객체 (KST 기준)         → _kstNow()
// ════════════════════════════════════════════════════════════════
