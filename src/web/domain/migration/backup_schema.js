...
(function(root, factory) {
  ...
  function legacyTaxType(name, explicit) {
  ...
  function migrateV6(source) {
    ...
    const accounts = names.map((displayName, index) => ({
      ...
      taxType: legacyTaxType(displayName, data.ACCT_TAX_TYPES?.[displayName]), active: true,
  ...
  function validate(source) {
    ...
    data.transactions.forEach((item, index) => { if (item.accountId && !ids.has(item.accountId)) throw new Error(`transactions[${index}]의 accountId 참조가 없습니다.`); });
  ...
  return { SCHEMA_VERSION, TAX_TYPES, SECRET_KEYS, stripSecrets, legacyTaxType, migrateV6, normalize, validate, createBackup, parseAndValidate };
});
