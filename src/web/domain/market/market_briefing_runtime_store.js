...
function mergeSnapshots(storage,snapshotApi,rows){const state=load(storage),incoming=Array.isArray(rows)?rows:[];const keys=new Set(incoming.map(row=>snapshotApi.snapshotKey(row.tradingDate,row.checkpoint)));state.snapshots=state.snapshots.filter(row=>!keys.has(row.key));for(const row of incoming)state.snapshots=snapshotApi.appendSnapshot(state.snapshots,row,{scenario:row.scenario});return save(storage,state);}
