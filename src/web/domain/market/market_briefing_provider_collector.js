...
function sourceFor(type){return type==='KOSPI'||type==='KOSDAQ'||type==='KOSPI200'?'TOSS':'YAHOO';}
...
function normalizeBenchmarkPoint(type,point,data){return {value:Number(point.value),tradingDate:String(point.date),source:sourceFor(type),status:'FINAL',session:'REGULAR',quality:'EOD',providerSymbol:String(data&&data.symbols&&data.symbols[type]||'')};}
