...
2026-08-18 사용자가 GAS v9.59 스프레드시트 메뉴에서 드라이런을 실행한 결과는 다음과 같다.
...
기존 `DIVDATA`에 같은 SEIBro 이벤트가 없으므로 파싱된 77건 전부가 신규이고, 정정·기존 일치가 0건인 결과는 집계 규칙과 일치한다. 18종목 모두 변경 예상인 것도 최초 SEIBro 연결 드라이런 결과와 일치한다.

결과창에 표시된 TTM 계산 12종목은 다음과 같다.

| 코드 | 종목명 | TTM 주당합계 | 건수 |
|---|---|---:|---:|
| `0046Y0` | ACE 미국배당퀄리티 | 179원 | 12건 |
| `0080G0` | KODEX 방산TOP10 | 5원 | 1건 |
| `091160` | KODEX 반도체 | 115원 | 1건 |
| `091170` | KODEX 은행 | 592원 | 12건 |
| `091180` | KODEX 자동차 | 713원 | 3건 |
| `133690` | TIGER 나스닥100 | 937원 | 4건 |
| `139220` | TIGER 200 건설 | 65원 | 1건 |
| `157500` | TIGER 증권 | 443원 | 1건 |
| `228790` | TIGER 화장품 | 29원 | 2건 |
| `232080` | TIGER 코스닥150 | 40원 | 1건 |
| `292150` | TIGER 코리아TOP10 | 322원 | 4건 |
| `305720` | KODEX 2차전지 | 18원 | 1건 |

v9.59 UI가 최대 12종목만 표시했으므로 나머지 6종목의 개별 TTM 합계는 당시 결과창에서 확인할 수 없었다. 다만 최상위 집계에서 대상 18종목의 77개 이벤트가 모두 파싱됐고 드라이런 자체의 오류나 쓰기는 없었다. 2단계 구조·증분 분류·TTM 계산 실행은 성공했으며, 당시 확인되지 않은 것은 화면에 생략된 6종목의 개별 표시값이다.

v9.60에서는 이 고정 표시 제한을 제거했다. 드라이런이 실행 시점의 `comparisons` 길이로 전체 확인 대상 수와 페이지 수를 계산하고, 10종목씩 결과창을 이어서 표시한다. 따라서 대상이 18개이면 2개 상세 결과창, 이후 19개나 그 이상으로 늘어나면 필요한 수만큼의 상세 결과창이 자동으로 표시된다. 변경 예상 종목뿐 아니라 기존 값과 일치하는 종목도 포함해 조회 대상 전체의 코드, 종목명, TTM 주당합계, 이벤트 건수, 변경 여부를 확인할 수 있다.

## 3단계 운영 반영

2026-08-18 사용자가 제공한 2단계 완료 화면에서 대상 ETF 18개, 분배금 77건, 신규 77건, 정정 0건, 기존 일치 0건, `DIVDATA` 변경 예상 18종목, 시트 및 `DIVDATA` 수정 없음이 확인됐다. 화면에 표시된 집계에는 오류 상태가 없고 앞선 1단계에서도 18종목 전체의 네 가지 오류가 0건이었으므로 3단계 기능을 추가했다.

v9.61의 3단계 메뉴는 저장 직전에 1단계 SEIBro 진단을 다시 실행한다. `NOT_FOUND`, `REQUEST_ERROR`, `PARSE_ERROR`, `MAPPING_ERROR`가 하나라도 있거나 진단 대상 수와 저장 비교 대상 수가 다르면 아무것도 저장하지 않는다. 검증 성공 시 Script Lock 안에서 기존 `ETF분배금이력`을 복합 키(종목코드, ISIN, 기준일, 지급일)로 증분 병합하고 `DIVDATA`를 갱신한다. 종목코드는 `_cleanCode()`를 거쳐 문자열로 저장하며 기존 이력과 `MANUAL` 이벤트는 보존한다. 두 저장 중 오류가 발생하면 저장 전 설정과 이력 시트 내용으로 복구한다.

### 3단계 실제 운영 결과

2026-08-18 사용자가 GAS v9.61의 3단계 운영 반영을 실행한 완료창에서 다음 결과를 확인했다.
...
함께 제공된 `ETF분배금이력` 시트 화면은 헤더 1행과 데이터 77행(마지막 행 번호 78)을 보여준다. 화면에서 종목코드, ISIN, 종목명, 기준일, 지급일, 주당분배금, 수집일시, 원본소스 열이 생성됐고 원본소스가 `SEIBRO`로 저장된 것을 확인했다. 혼합형 코드 `0046Y0`, `0080G0`도 문자열 형태로 표시돼 영문과 leading zero가 보존됐다.

따라서 최초 운영 저장은 완료된 것으로 판정한다. 다만 제공된 화면만으로는 웹 배당 탭의 화면 계산 결과와 동일 범위 재실행 시 `신규 0건 / 기존 일치 77건`이 되는지는 확인할 수 없다. 이 두 항목은 별도의 화면 또는 재실행 결과가 필요하다.

## 배당 탭 SEIBro 복원 보완

3단계 저장 후 제공된 웹 배당 탭 화면은 9개 종목만 표시했고, 상태 문구도 주식은 공공데이터 우선·ETF는 GOOGLEFINANCE 또는 수동 입력을 사용한다고 안내했다. 코드 확인 결과 배당 탭은 렌더링할 때마다 `_autoFetchDiv()`를 호출했고, 이 함수가 보유 종목 전체를 공공데이터/GOOGLEFINANCE로 다시 조회한 뒤 결과를 `DIVDATA`에 저장하고 있었다. 따라서 GAS에서 저장한 SEIBro ETF 결과를 탭 진입 자동조회가 교체할 수 있는 경로가 실제로 존재했다.

v9.62에서는 탭 진입과 수동 `배당금 불러오기` 실행 전에 `getDividendSettings`를 다시 호출한다. `source=SEIBRO`이고 이벤트가 존재하는 ETF는 기존 공공데이터/GOOGLEFINANCE 조회에서 제외하며, 화면 안내도 ETF가 GAS의 SEIBro 이력을 사용한다고 표시한다. 또한 아직 구버전 웹이 열려 있는 경우를 대비해 GAS `handleSaveDividendSettings()`가 기존 SEIBro 값을 `PUBLIC_DATA` 또는 `GOOGLEFINANCE` 값으로 교체하지 않도록 서버 보호를 추가했다. 명시적인 `SEIBRO` 갱신과 사용자 `MANUAL` 편집은 계속 허용한다.

### 배당 탭 자동 증분 갱신

v9.63에서는 배당 탭 진입 시 웹이 POST `refreshEtfDividends`를 호출한다. GAS는 현재 보유 ETF와 TTM 보유·거래 ETF를 동적으로 다시 선정하고, 대상 전체의 `DIVDATA`가 오늘 날짜의 `SEIBRO` 데이터인지 먼저 확인한다. 전부 최신이면 SEIBro 외부 요청과 시트 쓰기를 생략하고 현재 `DIVDATA`를 반환한다. 신규 ETF가 있거나 하루가 지났으면 기존 3단계와 같은 전체 오류 검증을 수행하고, 오류 0건일 때만 `ETF분배금이력` 및 `DIVDATA`를 증분 갱신한 뒤 웹에 반환한다.

따라서 v9.63 배포 후에는 정상 운영 중 3단계 메뉴를 매번 수동 실행할 필요가 없다. 자동 갱신이 실패하면 마지막 정상 `DIVDATA`를 다시 불러오며 공공데이터/GOOGLEFINANCE가 기존 SEIBro ETF 값을 덮어쓰지 않는다. 수동 3단계는 장애 확인, 즉시 강제 재수집 또는 운영 점검 용도로 유지한다.
