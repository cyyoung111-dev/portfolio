...
GAS v9.59를 새 버전으로 배포하고 스프레드시트를 새로고침한 뒤 다음 메뉴를 실행한다.

```text
📊 포트폴리오 → 🛠️ 유지보수 → 🧮 SEIBro ETF 2단계 드라이런
```

첫 안내창에서 `확인`을 누른 뒤 완료 결과창의 다음 항목을 확인한다.
...
6. `DIVDATA` 변경 예상
7. 종목별 `TTM` 금액과 건수
8. `시트 수정: 없음`, `DIVDATA 수정: 없음`

결과창 스크린샷 한 장을 전달하면 2단계 결과를 판정할 수 있다. 드라이런은 전체 분배금 행을 `date`, `payDate`, `amount`, `source=SEIBRO` 이벤트로 정규화하고 `ESTM_STDPRC` 합계를 `ttmPerShare`로 계산하지만 시트나 설정을 저장하지 않는다.
